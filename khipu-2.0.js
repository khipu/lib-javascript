 ...
 *
 */
var kh = jQuery.noConflict(false);
...
KhipuLib = {
	...
	startKhipu: function (url, paymentId, readyForTerminal, selector) {
		...
		KhipuLib.paymentId = paymentId;
		KhipuLib.url = url;
		KhipuLib.readyForTerminal = readyForTerminal;
		KhipuLib.selector = selector;

		if (!KhipuLib.readyForTerminal) {
			KhipuLib.sendToKhipu();
			return;
		}
	...
	sendToKhipu: function () {
		document.location.href = KhipuLib.url;
	},
	...
	startKhipuApp: function () {
		if (KhipuLib.isChrome()) {
			KhipuLib.sendEvent('startKhipuEvent', KhipuLib.paymentId + '|' + false)
			KhipuLib.showWaitAppModal();
			KhipuLib.verifyKhipuInstalled = setTimeout(KhipuLib.sendToKhipu, 10000);
		} else {
			var khPlugin = document.getElementById('khPlugin');
			khPlugin.callKH(KhipuSettings.serverUrl + 'payment/getPaymentData/' + KhipuLib.paymentId)
		}
	},
